...
bool isDestroy()
{
	//selain untuk mengecek apakah snake sudah kalah
...
char getkey()
//fungsi untuk mendapatkan tombol yang ditekan
{
	for(int i=8;i<=222;i++)
{
	if(GetAsyncKeyState(i)==-32767)
	{
		switch(i)
		{
			...
			break;//jka tombol yang ditekanadalah kanan
		}
	}
}
...
void random_makanan()
{
	...
	//mencegah agar makanan tidak diluar batas
...
bool isEaten()
{
	if(iX==makX&&iY==makY)return true;
	else return false;
}
...
void cBorder()//Fungsi untuk membuat garis tepi game
{
	...
	int main()//fungsi Utama
	{
		...
		while(!(isDestroy()))//selama snake belum rusak atau kalah
		{
			...
			if(isEaten())//mencek apakah makanan telah dilahap oleh snake
			{
